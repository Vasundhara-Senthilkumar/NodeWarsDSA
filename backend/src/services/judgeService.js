/**
 * Real judge service for NodeWars.
 * Evaluates user code against problem test cases by parsing inputs,
 * running the function in an isolated execution scope, and comparing outputs.
 */

function parseInput(inputStr) {
  if (typeof inputStr !== 'string') return [inputStr];
  let cleaned = inputStr.trim();
  // Strip param assignment prefixes like "energy = ", "speed = ", "grid = "
...
function executeUserCode(code, args, starterCode = '') {
  let funcName = null;
...
function runSimulatedJudge({ code, language, problem, mode = 'submit' }) {
...
function getHint(problem, unlockedCount = 0) {
  const hints = problem.hints || [];
  if (!hints.length) return 'No hints available for this problem.';
  const idx = Math.min(unlockedCount, hints.length - 1);
  return hints[idx];
}

function getScanFeedback(code = '') {
  const tips = [];
  if (!code.includes('null') && !code.includes('None') && !code.includes('empty')) {
    tips.push('Watch empty / null input edge cases.');
  }
  if (!/for|while|map|reduce/.test(code)) {
    tips.push('Your solution may be missing an iteration / traversal step.');
  }
  if (code.length < 100) {
    tips.push('Implementation looks incomplete — cover all constraints.');
  }
  if (!tips.length) {
    tips.push('Scan clear: no obvious edge-case traps detected.');
  }
